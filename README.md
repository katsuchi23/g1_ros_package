...
This workspace contains all ROS 2 packages required to run the Unitree G1 navigation pipeline with a Livox Mid360 LiDAR. It is the robotics-side counterpart to the `instruction-decomposed-semantic-navigation` stack.
...
Converts the Livox custom message format into a standard `sensor_msgs/PointCloud2` message. The output cloud is stamped in the `body` frame and published on `/livox/lidar/pcd2` (and also on `/converted_pointcloud2`). This is required because FAST-LIO and most ROS tools expect standard PointCloud2, not the Livox-proprietary format.
...
ros2 launch fast_lio_localization localization.launch.py \
  map:=/home/ntu/reynaldy_ws/unitree_g1_control/ros2_ws/src/mapping/FAST_LIO_ROS2/PCD/rrc2.pcd
```
...
Launches the full Nav2 navigation stack (costmaps, planner, controller) configured for the G1 robot, along with the ZMQ IPC bridge nodes that connect to the `instruction-decomposed-semantic-navigation` stack.
...
- **`ros_ipc_bridge.py`** — bundles robot pose and sends it over ZMQ for the semantic nav stack.
...
|------|-------------|---------|
| `map → odom` | `transform_fusion.py` | Global localization offset. Updated by the FAST-LIO global localization node to align the local FAST-LIO odometry with the pre-built PCD map. |
