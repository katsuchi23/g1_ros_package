...
class RobotPoseSender(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            f"RobotPoseSender: streaming TF at 50 Hz: "
    ...
    def publish_pose(self):
        ...
        map_base = payload["tfs"].get("map_base")
        if map_base is not None:
            ...
            if map_base_stamp == self._last_map_base_stamp:
                ...
                elif (now - self._map_base_frozen_since) >= 0.25 and (now - self._last_stale_warn_time) >= 1.0:
                    self.get_logger().warn(
        ...
        try:
            self.pub.send_string("TF " + json.dumps(payload), flags=zmq.NOBLOCK)
        except zmq.Again:
            pass
